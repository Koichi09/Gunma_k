...
class ReplayBuffer:
    ...
    def sample(self, batch_size, stage=0):
        ...
        if len(self.memory) > 0:
            ...
            stage_weights.append(0.6)
        
        # 過去のステージ（重み: 0.4を均等分配）
        past_weight = 0.4 / max(1, stage) if stage > 0 else 0
...
class DQNAgent:
    ...
    def set_stage(self, stage):
        ...
        print(f"Stage {stage}: Learning rate adjusted to {new_lr:.2e}")
...
if __name__ == "__main__":
    ...
    MODEL_SAVE_PATH = "models/dqn_lstm_model.pt"
    SAVE_FREQUENCY = 1000  # 何エピソードごとに保存するか
    
    # wandb設定
    USE_WANDB = True
    WANDB_PROJECT = "dqn-lstm-minigrid"
    ...
    if USE_WANDB:
        wandb.init(
            ...
            name=f"dqn_lstm_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
